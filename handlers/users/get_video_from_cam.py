...
@dp.callback_query_handler(cameras_callback_data.filter(function='video', kab='KabNone'), is_admin=True)
...
@dp.callback_query_handler(cameras_callback_data.filter(function='video', cam_name='CamNone'), is_admin=True)
...
async def get_camera_photo(call: CallbackQuery, callback_data: dict):
    ...
    out = cv2.VideoWriter(f"{callback_data.get('cam_name')}.avi", cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 10, (frame_width, frame_height))
    while datetime.datetime.now() - cur_time < datetime.timedelta(minutes=1):
        ret, frame = cap.read()
        out.write(frame)
    ...
    cap.release()
    out.release()
    video = open(f"{callback_data.get('cam_name')}.avi", 'rb')
    await dp.bot.send_video(
        chat_id=call.message.chat.id,
        video=video
    )
    video.close()
    os.remove(f"{callback_data.get('cam_name')}.avi")
